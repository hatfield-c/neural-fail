	...
	def Test(self, pipe_id):
		...
		for a in range(len(ablations)):
			...
			for pipe_id in CONFIG.pipelines:
				...
				ax1.scatter(valid_truth[:, 0], valid_truth[:, 1], color = "green", alpha = 0.4)
				ax1.plot(truth[:, 0], truth[:, 1], linestyle = 'dashed', color = "black", alpha = 0.4)
				
				ax.plot(np.arange(perror.shape[0]) / perror.shape[0], perror, label = model_id)
				ax1.plot(poses[:, 0], poses[:, 1], label = model_id)
